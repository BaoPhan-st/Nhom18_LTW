            ...
            <%-- FORM --%>
                <div class="form-box">
                    ...
                    <form action="${pageContext.request.contextPath}/admin/products" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <select name="brandId">
                                <c:forEach var="b" items="${brands}">
                                    <option value="${b.id}" <c:if test="${isEdit && product.brandId == b.id}">
                                        selected
                                        </c:if>>
                                        ${b.name}
                                    </option>
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <select name="available">
                                        <option value="true" <c:if test="${!isEdit || product.available}">selected
                                            </c:if>>
                                            Đang bán
                                        </option>
                                        <option value="false" <c:if test="${isEdit && !product.available}">selected
                                            </c:if>>
                                            Ngừng bán
                                        </option>
                                    </select>
                        ...
                        <button type="submit" class="btn-submit" <c:if test="${product.discontinue}">disabled</c:if>>
                        ...
                        <%-- SEARCH & FILTER --%>
                            <div class="filter-bar">
                                <form method="get" action="${pageContext.request.contextPath}/admin/products">
                                    ...
                                    <input type="text" name="id" placeholder="ID sản phẩm..." value="${param.id}" />
                                    ...
                                    <select name="brandId">
                                        ...
                                        <c:forEach var="b" items="${brands}">
                                            <option value="${b.id}" <c:if test="${param.brandId == b.id}">
                                                selected
                                                </c:if>
                                                >
                                                ${b.name}
                                                    ...
                                                    <td>
                                                        <c:choose>
                                                            <c:when test="${p.discontinue}">
                                                                <span class="status CANCELLED">Đã xoá</span>
                                                            </c:when>
                                                            <c:when test="${!p.available}">
                                                                <span class="status PENDING">Ngừng bán</span>
                                                            </c:when>
                                                            <c:otherwise>
