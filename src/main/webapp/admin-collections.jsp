    ...
    <form action="${pageContext.request.contextPath}/collection" method="post">
        ...
        <div class="form-group">
            ...
            <select name="ruleSet_type">
                <option value="manual" <c:if test="${collection.ruleSet_type == 'manual'}">selected</c:if>>Manual
                </option>
                <option value="automatic" <c:if test="${collection.ruleSet_type == 'automatic'}">selected</c:if>>
                    Automatic
                </option>
            </select>
        </div>
