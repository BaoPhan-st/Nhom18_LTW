                    ...
                    <c:choose>
                        <c:when test="${p.mainImageUrl.startsWith('http')}">
                            <img src="${p.mainImageUrl}"
                                 loading="lazy" alt="${p.name}" class="image-contain" />
                        </c:when>
                        <c:otherwise>
                            <img src="${pageContext.request.contextPath}${p.mainImageUrl}"
                                 loading="lazy" alt="${p.name}" class="image-contain" />
                        </c:otherwise>
                    </c:choose>
...
<div class="pagination">
    ...
    <!-- Prev -->
    <button class="page-btn prev-btn ${page==1 ? 'disabled' : ''}"
            data-page="${page-1}" ${page==1 ? 'disabled' : ''}>&laquo;</button>
    ...
    <!-- Next -->
    <button class="page-btn next-btn ${page==totalPages ? 'disabled' : ''}"
            data-page="${page+1}" ${page==totalPages ? 'disabled' : ''}>&raquo;</button>
</div>
